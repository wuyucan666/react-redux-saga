...
//   }
  ...
  export function CustomBraftEditor(props){
    ...
    useEffect(() => {
      ...
      const {actions} = props
      actions.EmptyList()
      actions.InvitationAdditionalDetails(parmas);
    ...
    const handlePage = (page) => {
      ...
      // console.log(val);
      const {actions} = props
      actions.InvitationAdditionalDetails(val);
    }
    ...
    return (
      <div>
      <IceContainer >
      <Link target="_blank" to={`/Activity/InvitationDetails?uuid=${parmas.uuid}`}  style={styles.nav} >
        基础奖励
        </Link>
        <Link target="_blank" to={`/Activity/InvitationAdditionalDetails?uuid=${parmas.uuid}`} style={styles.nav1} >
        额外奖励
        </Link>
      </IceContainer>
      <IceContainer>
